 ...
 * A model can belong to a saved ProviderProfile; selecting it activates the
 * complete connection (provider, base URL, API key and model), not just a
 * model string.
 */
...
export function getComposerModels(): string[] {
  const profileModels = getProviderProfiles().filter((p) => p.enabled).map((p) => p.model);
  const current = String(configManager.get('agent-k.provider.model') || '').trim();
  let ids = [...profileModels, ...readStringList(AVAILABLE_KEY), ...readStringList(LEGACY_REGISTERED_KEY)];
...
/** Activate the complete saved connection for a model before using it. */
export function persistProviderModel(model: string): void {
  const profile: ProviderProfile | undefined = findProviderProfileForModel(model);
  if (profile) {
  ...
  configManager.update({ [AVAILABLE_KEY]: nextModels, [LEGACY_REGISTERED_KEY]: nextModels });
  persistToHost({ [AVAILABLE_KEY]: nextModels, [LEGACY_REGISTERED_KEY]: nextModels });
}
...
function testViaExtensionHost(baseUrl: string, apiKey: string, model?: string): Promise<{ ok: boolean; status?: number; detail: string; modelIds?: string[] }> {
  return new Promise((resolve, reject) => {
    ...
    api.postMessage({ type: 'provider.test', requestId, baseUrl, apiKey: apiKey || undefined, model: model || undefined });
  });
}

async function testViaDirectFetch(baseUrl: string, apiKey: string, model?: string): Promise<{ ok: boolean; status?: number; detail: string; modelIds?: string[] }> {
  const root = baseUrl.replace(/\/$/, '');
  const response = await fetch(`${root}/v1/models`, { headers: apiKey ? { Authorization: `Bearer ${apiKey}` } : {}, signal: AbortSignal.timeout(8000) });
  if (!response.ok) {
    let detail = `HTTP ${response.status}`;
    if (response.status === 401) detail += ' — Unauthorized. Check API Key, or use a local endpoint that does not require auth.';
  ...
  const detail = model && found ? `OK — model "${model}" listed (${modelIds.length} models)` : modelIds.length > 0 ? `OK — ${modelIds.length} models loaded into Composer` : 'OK — server reachable (no models in list).';
  return { ok: found || modelIds.length > 0 || !model, status: response.status, detail, modelIds };
}

export type ProviderModelsResult = { ok: boolean; status?: number; detail: string; modelIds: string[] };

export async function fetchProviderModels(opts?: { baseUrl?: string; apiKey?: string; model?: string }): Promise<ProviderModelsResult> {
  ...
  const model = String(opts?.model ?? configManager.get('agent-k.provider.model') ?? '');
  if (!baseUrl) return { ok: false, detail: 'Base URL is empty', modelIds: [] };
  try {
    let result: { ok: boolean; status?: number; detail: string; modelIds?: string[] };
    const api = getVsCodeApi();
    if (api) result = await testViaExtensionHost(baseUrl, apiKey, model || undefined);
    else result = await testViaDirectFetch(baseUrl, apiKey, model || undefined);
    return { ok: result.ok, status: result.status, detail: result.detail, modelIds: result.modelIds || [] };
  } catch (e: unknown) {
    const msg = e instanceof Error ? e.message : String(e);
    try {
      const fallback = await testViaDirectFetch(baseUrl, apiKey, model || undefined);
      return { ok: fallback.ok, status: fallback.status, detail: fallback.detail, modelIds: fallback.modelIds || [] };
    } catch (inner: unknown) {
      return { ok: false, detail: inner instanceof Error ? inner.message : msg || 'Connection failed', modelIds: [] };
    }
  }
}
