...
import { PROVIDER_PRESETS, manualModelPresetsForType } from '../../providers/providerPresets';
            ...
            {showManualModels && editingId ? (
              <div className="settings-field">
                ...
                <div className="preset-row">
                  {manualModelPresetsForType(effectiveType).map((id) => (
                    <button key={id} type="button" className="settings-btn secondary settings-btn--tiny" onClick={() => handleAddManual(editingId, id)}>{id}</button>
                  ))}
                </div>
              </div>
            ) : null}
