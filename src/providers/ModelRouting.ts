...
function read(): AIConfiguration {
  const raw = configManager.get(AI_CONFIGURATION_KEY);
...
function write(value: AIConfiguration): void {
  configManager.update({ [AI_CONFIGURATION_KEY]: value });
  try {
    const api = (window as any).__vscodeApi || (window as any).acquireVsCodeApi?.();
    if (api?.postMessage) api.postMessage({ type: 'config.update', values: { [AI_CONFIGURATION_KEY]: value } });
    else window.parent.postMessage({ type: 'config.update', values: { [AI_CONFIGURATION_KEY]: value } }, '*');
  } catch {
    // Non-webview context.
  }
}
...
export function upsertModelProfile(input: Omit<Partial<ModelProfile>, 'createdAt' | 'updatedAt'> & {
  providerProfileId: string;
  name: string;
  id?: string;
}): ModelProfile {
...
export function setRoutingRule(role: ModelRole, primaryProfileId: string, fallbackProfileIds: string[] = [], fallback = DEFAULT_FALLBACK): RoutingRule {
  const config = read();
...
export function resolveProviderProfile(profile: ModelProfile): ProviderProfile | undefined {
  return getProviderProfiles().find((provider) => provider.id === profile.providerProfileId && provider.enabled);
}
