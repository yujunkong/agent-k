/**
 * StackTraceParser - 에러 스택 트레이스 파싱 (C1-T16)
 * 
 * 파일:라인:컬럼 패턴 추출, 다양한 언어 지원
 */
...
export function getContextFiles(text: string, contextLines = 3): Array<{ file: string; startLine: number; endLine: number }> {
  const frames = parseStackTrace(text);
  if (frames.length === 0) return [];

  // Group by file and get line ranges
