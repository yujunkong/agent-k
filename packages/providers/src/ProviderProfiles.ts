/**
 * PROVIDER-004 — Per-model provider profiles (host activation projection).
 * Storage via injected ProviderConfigStore (no vscode / core ConfigManager).
 */
...
export interface ProviderProfile {
  ...
  createdAt: number;
  updatedAt: number;
  /** Link back to ProviderConnections registry (optional for legacy rows) */
  connectionId?: string;
}

function readProfiles(): ProviderProfile[] {
  const raw = getProviderConfigStore().get(PROVIDER_PROFILES_KEY);
  if (!Array.isArray(raw)) return [];
  return raw.filter((p): p is ProviderProfile => {
    if (!p || typeof p !== 'object') return false;
    const o = p as ProviderProfile;
    return (
      typeof o.id === 'string' &&
...
function writeProfiles(profiles: ProviderProfile[]): void {
  getProviderConfigStore().update({ [PROVIDER_PROFILES_KEY]: profiles });
}
...
export function getActiveProviderProfileId(): string | null {
  const id = String(getProviderConfigStore().get(ACTIVE_PROFILE_KEY) || '').trim();
  return id || null;
}
...
export function removeProviderProfile(profileId: string): boolean {
  ...
  if (getActiveProviderProfileId() === profileId) {
    ...
    getProviderConfigStore().update({ [ACTIVE_PROFILE_KEY]: nextActive });
...
export function activateProviderProfile(profileId: string): ProviderProfile | undefined {
  ...
  const apiKeys = getProviderConfigStore().get('agent-k.provider.apiKeys');
