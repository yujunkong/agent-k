/**
 * MODEL-001 — Unified model catalog across connections.
 * Composer shows this list; runtime pick uses ModelResolver (R-001).
 */
...
/** Build deduped catalog from all saved connections (MODEL-006 discovery surface). */
export function listUnifiedModels(): UnifiedModel[] {
  ...
  for (const conn of connections) {
    ...
    for (const original of connectionModelIds(conn)) {
      ...
      if (local) {
        bucket.local = true;
        // Prefer local original id for display ordering
...
/** Connections that advertise the given model id (any original spelling). */
