/**
 * PROVIDER-003 — Connection registry (Name + Base URL + API Key).
 * Models attach as discovered/manual lists; profiles sync for host activation.
 *
 * Custom OpenAI Compatible: use addCustomOpenAICompatibleConnection() or
 * upsertProviderConnection({ type: 'litellm', ... }).
 */
...
import { classifyProbeResult, effectiveHealthStatus, type ProviderHealthStatus } from './providerStatus';
...
export interface ProviderConnection {
  ...
  name: string;
  type: ProviderType;
  /** auto = Base URL detect; manual = Advanced override */
  ...
  /** Lower = higher priority (drag order). */
  priority: number;
  createdAt: number;
  updatedAt: number;
}
...
function readConnections(): ProviderConnection[] {
  const raw = getProviderConfigStore().get(PROVIDER_CONNECTIONS_KEY);
  ...
  return raw.filter(isConnection).map((c) => ({
    ...
    extraHeaders:
      c.extraHeaders && typeof c.extraHeaders === 'object' ? c.extraHeaders : undefined,
  }));
}

function writeConnections(connections: ProviderConnection[]): void {
  getProviderConfigStore().update({ [PROVIDER_CONNECTIONS_KEY]: connections });
}

export function getPreferUserOrder(): boolean {
  return Boolean(getProviderConfigStore().get(PREFER_USER_ORDER_KEY));
}

export function setPreferUserOrder(value: boolean): void {
  getProviderConfigStore().update({ [PREFER_USER_ORDER_KEY]: value });
}

export function connectionModelIds(
  conn: Pick<ProviderConnection, 'discoveredModels' | 'manualModels'>,
): string[] {
...
/** Project connection models onto host-activation ProviderProfiles */
...
export function upsertProviderConnection(
  ...
  const conn: ProviderConnection = {
    ...
    extraHeaders:
      input.extraHeaders !== undefined ? input.extraHeaders : existing?.extraHeaders,
...
/**
 * Add a custom OpenAI Compatible connection (user request).
 * Forces type `litellm` unless autoDetectType is true.
 */
export function addCustomOpenAICompatibleConnection(input: {
  name: string;
  baseUrl: string;
  apiKey?: string;
  extraHeaders?: Record<string, string>;
  /** When true, keep auto-detected type if URL maps to openai/ollama/etc. */
  autoDetectType?: boolean;
}): ProviderConnection {
  const baseUrl = input.baseUrl.replace(/\/$/, '');
  let type: ProviderType = 'litellm';
  let typeSource: 'auto' | 'manual' = 'manual';
  if (input.autoDetectType) {
    const detected = detectProviderType(baseUrl);
    type = detected.type;
    typeSource = 'auto';
  }
...
export function findConnectionByEndpoint(
  baseUrl: string,
  type?: string,
): ProviderConnection | undefined {
...
export function findConnectionForProfile(
  profile: ProviderProfile,
): ProviderConnection | undefined {
  if (profile.connectionId) {
    return readConnections().find((c) => c.id === profile.connectionId);
  }
  return readConnections().find(
    (c) =>
      c.type === profile.type &&
      c.baseUrl.replace(/\/$/, '') === profile.baseUrl.replace(/\/$/, ''),
  );
}

/** One-shot migrate legacy per-model profiles → connection-per-endpoint. */
