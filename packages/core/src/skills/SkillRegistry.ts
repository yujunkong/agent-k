/**
 * SkillRegistry — SKILL-001/002: Skills / Pinned Skills 관리
 * (v2.1 C7-T19 / PRD-28 동작 동등 이식)
 *
 * skills/*.md 로드·리로드, 핀한 스킬 주입, 핀 해제 즉시 중단,
 * Tier A 캡 초과 경고, 시크릿 평문 거부/마스킹.
 * TOOL-013 SkillTool(on-demand 로드)과 별개 — registry는 pin/inject 담당.
 */
...
export class SkillRegistry {
  ...
  private scanSecrets(skill: SkillDefinition): void {
    ...
    for (const pattern of secretPatterns) {
      ...
      if (match) {
        // Replace with masked version (no plaintext logging)
...
/**
 * SKILL-003 — Skill feature flag gate (McpPermissions 스타일).
 * enabled === false면 pin/inject·registry 접근을 모두 off.
 */
export interface SkillFeaturePolicy {
  enabled?: boolean;
}

export function checkSkillFeature(policy?: SkillFeaturePolicy): { allowed: boolean; reason?: string } {
  if (policy?.enabled === false) {
    return { allowed: false, reason: 'Skills system is disabled by feature flag' };
  }
  return { allowed: true };
}
