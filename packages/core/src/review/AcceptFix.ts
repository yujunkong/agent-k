/**
 * AcceptFix — REVIEW-003: Finding 선택 수정 마이크로 Agent 실행 (v2.1 C7-T13 동작 동등 이식)
 */
...
export class AcceptFix {
  ...
  async validateLint(filePath: string): Promise<boolean> {
    const lintErrors = await this.lintRunner.runLint([filePath]);
    return lintErrors.filter(e => e.severity === 'error').length === 0;
  }
}
