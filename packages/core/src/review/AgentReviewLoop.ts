/**
 * AgentReviewLoop — REVIEW-001/002: git diff 수집 → 정적 힌트 + LM 리뷰 프롬프트
 * (v2.1 C7-T11 / ADDON-T14 동작 동등 이식; execSync shell pipe → execFile argv + JS 스캔)
 */
...
export class AgentReviewLoop {
  ...
  reviewDiff(baseBranch: string = 'HEAD'): ReviewResult {
    const diffOutput = this.execGit(['diff', baseBranch, '--stat']);
  ...
  private execGit(args: string[], maxBuffer = 2 * 1024 * 1024): string {
    try {
  ...
  private generateStaticHints(): ReviewFinding[] {
    ...
    try {
      const diff = this.execGit(['diff', 'HEAD'], 10 * 1024 * 1024);
      if (!diff) return findings;

      // Check for TODO/FIXME markers in diff
      if (/^\+.*\b(TODO|FIXME|HACK|XXX)\b/m.test(diff)) {
        findings.push({
      ...
      if (/^\+.*console\.(log|debug)/m.test(diff)) {
        findings.push({
          id: `static-${findings.length + 1}`,
          file: 'multiple',
          line: 0,
      ...
      const diffStat = this.execGit(['diff', 'HEAD', '--stat']);
