/**
 * CTX-011 — taskContextStrategy ported from v2.1 `src/prefetch/taskContextStrategy.ts`.
 * ADDON-T04: pure task-type → required/optional IDE context keys (unit-test friendly).
 */
...
/** Infer task type from user text + mode hints (v2.1 Mode → AgentMode). */
export function inferTaskType(userMessage: string, mode?: AgentMode): TaskType {
 ...
 * Drops optional when over budget (required may still exceed — caller truncates).
 */
