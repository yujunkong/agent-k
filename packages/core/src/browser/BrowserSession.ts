/**
 * BrowserSession — BROWSER-001: 영구 브라우저 세션 관리 (v2.1 C7-T02 동작 동등 이식)
 *
  ...
  async init(): Promise<void> {
    if (!this.browser) {
      try {
        ...
        const pw: any = await import('playwright' as never);
        this.browser = await pw.chromium.launch({
          headless: true,
          args: ['--disable-dev-shm-usage', '--no-sandbox']
        });
      } catch {
        // Playwright not available — createSession will throw with guidance
      }
    }
  }

  /**
   * Create a new browser session (requires Playwright installed)
   */
