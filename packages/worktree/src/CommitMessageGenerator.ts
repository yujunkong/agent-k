/**
 * CommitMessageGenerator — SCM-001: git diff 기반 커밋 메시지 / PR 설명 생성
 * (v2.1 C7-T24 동작 동등 이식; git via runGit/tryGit execFile argv — cross-platform)
 *
...
export class CommitMessageGenerator {
  ...
  async commit(
    ...
    try {
      ...
      const result = runGit(['commit', '-m', fullMessage], {
        cwd: this.repoRoot,
  ...
  /**
   * git 명령어를 실행하고 stdout 을 반환한다.
   * 명령어가 실패하면 빈 문자열을 반환한다.
   */
  private execGit(args: string[]): string {
    return tryGit(args, { cwd: this.repoRoot, maxBuffer: 2 * 1024 * 1024 }) ?? '';
  }
