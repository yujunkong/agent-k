/**
 * WT-007 / WT-013 (domain) — unified diff → per-file rows for review UI.
 * Chat-ui CONV-011 already consumes a sibling parser; this is the worktree package copy.
 */
...
export type WorktreeDiffPreview = {
  diff?: string;
  files?: string[];
...
export function buildWorktreeDiffFiles(preview?: WorktreeDiffPreview): WorktreeDiffFile[] {
  if (!preview) return [];
  ...
  for (const raw of preview.files ?? []) {
    ...
    byPath.set(p, { path: p, status: 'M', additions: 0, deletions: 0, lines: [] });
  }

  for (const raw of preview.untrackedFiles ?? []) {
    const p = normalizeRepoPath(raw);
    if (!p || byPath.has(p)) continue;
    byPath.set(p, { path: p, status: '?', additions: 0, deletions: 0, lines: [] });
  }

  const ordered = [
    ...(preview.files ?? []).map(normalizeRepoPath),
    ...parsed.map((f) => f.path),
  ];
